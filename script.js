cvs = document.getElementById("canvas");
ctx = cvs.getContext('2d');

const width = window.innerWidth;
const height = window.innerHeight;

cvs.width = width;
cvs.height = height;

let points = [];
let maxpoints = 15;
let counter = 0;
let hit = 0;

for (let i = 0; i < maxpoints; i++) {
    newPoint("rgba(255,66,22,0.2)");
}

let mouse = new Vector2d();
let difference = new Vector2d();

window.addEventListener('click', (evt) => {
    mouse.dx = evt.clientX;
    mouse.dy = evt.clientY;

    for (var i = 0; i < points.length; i++) {
        difference.differenceVector(points[i].position, mouse);
        if (difference.magnitude < points[i].radius) {
            points[i].color = "rgba(33,66,255,0.2)";
            hit++
        };
    }
    console.log(hit);
    if (hit >= maxpoints) {
        hit = 0;
        counter = 0;
        points.splice(0, maxpoints)
        for (var i = 0; i < maxpoints; i++) {
            newPoint("rgba(255,99,66,0.2)");
        }
    }

})

function animate() {
    context.clearRect(0, 0, width, height);
    requestAnimationFrame(animate);


    for (let i = 0; i < points.length; i++) {
        points[i].draw(context);
    }
}

animate()


function rng(max) {
    let ans = Math.floor(Math.random() * max);
    return ans;
}

function newPoint(color) {
    let A = new Point(new Vector2d(rng(width), rng(height)), 50, color, "hallos");
    A.label = counter;
    counter++;
    points.push(A);
}